...
droplist=['utctime','utctime.1','latitude', 'longitude', 'FMISID', pred,'lat-1', 'lon-1', 'lat-2', 'lon-2', 'lat-3', 'lon-3', 'lat-4', 'lon-4','hour','dayOfYear','e-1','e-2','e-3','e-4','msl-1','msl-2','msl-3','msl-4','str-1','str-2','str-3','str-4','tcc-1','tcc-2','tcc-3','tcc-4','tlwc-1','tlwc-2','tlwc-3','tlwc-4','lsm-1','lsm-2','lsm-3','lsm-4']
cols_own=None

def set_variables(pred_arg):
    global pred, correl_pred, quantile_alpha, qpred, fname, obsfile, fscorepic, mdl_name, xgbstudy, cols_own, test_y, train_y
    pred = pred_arg
    ...
    split_file = f'/home/ubuntu/data/ML/models/OCEANIDS/best_split_{pred}_{harbor}_{starty}-{endy}.json'
    if os.path.exists(split_file):
        with open(split_file, 'r') as f:
            best_split = json.load(f)
            train_y = best_split['train_years']
            test_y = best_split['test_years']
    else:
        test_y=[2015,2019]
